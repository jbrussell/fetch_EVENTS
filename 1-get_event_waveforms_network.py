...
# 
# This version of the code pulls data only for a single network.
#
# ##### JBR - 2/3/18
...
for iev, ev in enumerate(cat_evts) :  
    ...
    for inet in range(0,len(inventory)):
        ...
        for ista in range(0,len(inventory[inet])) :
            stel = inventory[inet].stations[ista].elevation
            stla = inventory[inet].stations[ista].latitude
            stlo = inventory[inet].stations[ista].longitude
